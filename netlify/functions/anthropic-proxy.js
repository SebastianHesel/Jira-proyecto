 ...
 *
 * The browser sends the API key in x-api-key — this function just proxies it
...
const CORS = {
  ...
  'access-control-allow-headers': 'x-api-key,anthropic-version,content-type,accept',
}
...
export const handler = async (event) => {
  ...
  try {
    ...
    const apiPath = original.pathname.replace(/^\/api\/anthropic/, '') || '/'
    const targetUrl = `https://api.anthropic.com${apiPath}${original.search}`
    ...
    // Server-side calls don't need the browser-access header
    delete forwardHeaders['anthropic-dangerous-direct-browser-access']
